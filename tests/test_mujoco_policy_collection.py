...
class MujocoPolicyCollectionTest(unittest.TestCase):
    ...
    def test_collect_mujoco_policy_episode_writes_images_actions_and_jsonl(self) -> None:
        with TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(record["observation"]["state"]["board_after_flat"][0], 1)
            self.assertEqual(record["observation"]["state"]["supply_after"]["black"], 4)
            self.assertTrue(record["observation"]["state"]["safety"]["ok"])
            self.assertEqual(record["observation"]["action"]["attachment_mode"], "scripted_held_stone")
