...
Project goal: transfer a Gomoku-aware VLA, not a coordinate-following placement bot. Real camera inputs must support board-state inference and move choice before robot execution.
...
These are still baseline interfaces. The SO-101 collection path now uses an active stone body with a constraint-style gripper lock and records grasp/place validation, but it is not yet friction-only contact-stable MuJoCo grasping. The vision detector assumes a calibrated top-down camera view; real camera use still needs calibration, lens correction, lighting checks, and confidence/error handling.
